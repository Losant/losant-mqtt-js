...
module.exports = function(options) {
   ...
   */
  device.sendState = function(state, timeOrCb, cb) {
    var time = null;
   ...
   * time (optional): the time at which the state change is requested (defaults to now)
   * cb: the callback handling the response.
   */
  device.sendStateChangeRequest = function(deviceId, state, timeOrCb, cb) {
    var time = null;
    if(typeof(timeOrCb) === 'function')
    { cb = timeOrCb; }
    else
